...
def load_data(file_path):
    ...
    try:
        ...
        # FIX 1: Explicitly clean doc_text to remove non-ASCII characters upon load
        # (Kept this functionality from the original combined file)
        merged_data['doc_text'] = merged_data['doc_text'].astype(str).apply(
            lambda x: x.encode('ascii', 'ignore').decode('ascii')
        )
        # ----------------------------------------------------------------------------

        # --- FIX 2: Removed legacy code that referenced 'Product_normalized' and 'Name_x'.
        # The source CSV is now expected to have the correct columns directly.

        # --- FIX 3: Validation: Ensure the new required columns exist ---
        ...
        # --- FIX 4: Filter DataFrame to only contain the three required columns ---
...
# FIX: load_data now only returns merged_data and product_options
...
if st.session_state.page == 'p1':
    ...
    with st.sidebar:
        ...
        if st.button("Estimate Cost & Setup Pipeline", type="primary", key='p1_estimate_button'):
            ...
            elif target_product_input == product_options[0]:
...
# ==============================================================================
    ...
    with st.sidebar:
        ...
        product_1_input = st.selectbox("Product 1 Name", product_options, key='product_1_select_p2', index=0)
        product_2_input = st.selectbox("Product 2 Name", product_options, key='product_2_select_p2', index=0)
    ...
    if run_collection_button:
        ...
        if product_1_input == product_options[0] or product_2_input == product_options[0]:
